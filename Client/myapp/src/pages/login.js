...
const LoginPage = () => {
    ...
    const loginCheck = () => {
        ...
        if (user) {
            ...
            const userPermissions = permissions?.find((permissionListUser) => permissionListUser._id === user._id);

            if (!userPermissions) {
                return alert(`No permissions found to ${user.name}`);
            }

            sessionStorage.setItem('permissions', JSON.stringify(userPermissions.permissions));
